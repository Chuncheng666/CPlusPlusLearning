// QCCMultiThreadNBServer.cpp : Defines the entry point for the console application.
//
...
DWORD WINAPI QCCCommunicateToClientThread(LPVOID lparam)
{
	char buffer[BUF_SIZE];
	int errCode;

	SOCKET qCliSocket = (SOCKET)(LPVOID)lparam;
	while (true)
	{
		...
		cout << curTime << ", Recv from client: " << buffer << endl;
...
int main()
{
	...
	while (true)
	{
		...
		else
		{
			...
			DWORD qTheadId;
			CreateThread(NULL, NULL, QCCCommunicateToClientThread, (LPVOID)qCliSocket, 0, &qTheadId);
		}
	}
